...
## Comparing variance of the two scoring variables against each other
```{r, echo=TRUE}
vardata<-atplong %>%
group_by(Player) %>%
dplyr::summarise(
...
```
This shows me a very strong relationship between the 2 variables, and the scale of one compared to the other also explains the alpha value. The BP scoring variable having more margin in the scale allows the alpha to be smaller - with such a large scale, you don't need to inflate it any further (which is what the alpha does, the larger it is). On the other hand, the TPW scoring variable has less margin in the scale, and so that allows for the alpha value to be larger, hence the difference between alpha=1.714 for BP and 6.969 for TPW. 

## Determining good/strong indicators of a win
Looking for variables that might evidently signal a strong/dominant win in tennis: between the two scoring variables, I'm trying to determine which is better. I obtain the summaries of the models to get a better idea of the fits:
